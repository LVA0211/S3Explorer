...
Object Object::loadglTF(const char* path) {
	...
	std::string fileDir = pathStr.substr(0, pathStr.find_last_of('/') + 1);
	...
	unsigned int bufferViewInd = JSON.value("bufferView", 1);
	unsigned int count = JSON["count"];
	unsigned int accByteOffset = JSON.value("byteOffset", 0);
	std::string type = JSON["type"];

	json bufferView = JSON["bufferViews"][bufferViewInd];
	unsigned int byteOffset = bufferView
}
...
std::vector<GLuint> getIndices(json accessor,json JSON)
{
